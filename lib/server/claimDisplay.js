...
// import { env } from '../env.js'; // Removed to avoid env validation blocking
...
export async function handleClaimRequest(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Continue without user - we'll handle this below
    }
    ...
    console.log('[claimDisplay] Update payload:', updatePayload);
    ...
    console.log('[claimDisplay] Claim successful!');
    return res.status(200).json({ success: true, retailer_id: retailerId, business_id: businessId });
  } catch (error) {
    // Note: AuthError should not reach here since we handle auth errors internally now
