...
export async function handleClaimRequest(req, res) {
  ...
  try {
    ...
    // ✅ STEP 1: Check if UID exists (but don't fail if it doesn't!)
    const { data: uidRecord, error: uidError } = await supabaseAdmin
      .from('uids')
      .select('uid, is_claimed, affiliate_url, retailer_id, business_id')
      .eq('uid', uid)
      .maybeSingle();

    if (uidError) {
      console.error('[claimDisplay] UID fetch error:', uidError.message);
      return res.status(500).json({ error: 'Failed to check UID', details: uidError.message });
    }

    // ✅ CRITICAL FIX: If UID exists AND is already claimed, return conflict
    if (uidRecord && uidRecord.is_claimed) {
    ...
    // ✅ If UID doesn't exist OR exists but is unclaimed, we'll create/update it below
    const uidExists = uidRecord !== null;
    console.log('[claimDisplay] UID exists:', uidExists, '| Already claimed:', uidRecord?.is_claimed || false);
    ...
    // CRITICAL: affiliate_url must NEVER be null for commission tracking to work
    let affiliateUrl = uidRecord?.affiliate_url;

    if (!affiliateUrl) {
      if (!env.NEXT_PUBLIC_SHOPIFY_DOMAIN) {
        console.error('[claimDisplay] CRITICAL ERROR: NEXT_PUBLIC_SHOPIFY_DOMAIN not set');
        return res.status(500).json({
          error: 'Configuration error: Shopify domain not configured',
          details: 'Cannot generate affiliate URL without NEXT_PUBLIC_SHOPIFY_DOMAIN'
        });
      }

      // Generate affiliate URL with multiple tracking parameters for better capture
      // Uses both 'ref' (our custom tracking) and Shopify's landing_site_ref
      affiliateUrl = `https://pawpayaco.com/products/custom?ref=${uid}&utm_source=nfc&utm_medium=display&utm_campaign=${uid}`;
      console.log('[claimDisplay] Generated affiliate URL:', affiliateUrl);
    }

    // Validate that we have a valid URL
    if (!affiliateUrl || affiliateUrl.trim() === '') {
      console.error('[claimDisplay] CRITICAL ERROR: Could not generate valid affiliate_url');
      return res.status(500).json({
        error: 'Failed to generate affiliate URL',
        details: 'affiliate_url is required for commission tracking'
      });
    }

    // ✅ STEP 5: CREATE or UPDATE the UID
    const uidPayload = {
      uid: uid,
      is_claimed: true,
    ...
    if (user) {
      uidPayload.claimed_by_user_id = user.id;
    }

    if (businessId) {
      uidPayload.business_id = businessId;
    } else if (retailerRecord?.business_id) {
      uidPayload.business_id = retailerRecord.business_id;
    }

    console.log('[claimDisplay] UID payload:', JSON.stringify(uidPayload, null, 2));

    let uidOperation;
    if (!uidExists) {
      // ✅ CREATE new UID (this is the normal flow for first-time claim!)
      console.log('[claimDisplay] 🆕 Creating NEW UID:', uid);
      ...
      if (insertError) {
        console.error('[claimDisplay] Failed to create UID:', insertError.message);
        return res.status(500).json({ error: 'Failed to create UID', details: insertError.message });
      }

      uidOperation = 'created';
      console.log('[claimDisplay] ✅ UID created successfully:', uid);
    } else {
      // ✅ UPDATE existing unclaimed UID
      console.log('[claimDisplay] 🔄 Updating existing unclaimed UID:', uid);
      ...
      if (updateError) {
        console.error('[claimDisplay] Failed to update UID:', updateError.message);
        return res.status(500).json({ error: 'Failed to update UID', details: updateError.message });
      }

      uidOperation = 'updated';
      console.log('[claimDisplay] ✅ UID updated successfully:', uid);
    }
    ...
    if (user) {
      await logEvent(user.id, 'uid_claimed', {
        ...
        auto_created_retailer: !rawRetailerId && retailerRecord?.source === 'uid_claim_auto',
      });
    } else {
    ...
    return res.status(200).json({
      success: true,
      operation: uidOperation, // 'created' or 'updated'
      retailer_id: retailerId,
