    ...
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      <div className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors">
        ...
        <button
          ...
          className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-[4px] hover:bg-gray-200 transition-colors"
        >
          <svg
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-lg font-medium text-gray-900 truncate">{retailer.name}</h3>
            {summary.uid_count > 0 && (
        ...
        {summary.pending_count > 0 && (
          <motion.button
            ...
            className="px-6 py-3 rounded-[4px] bg-gradient-to-r from-green-400 to-emerald-500 text-white font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
          >
        ...
        {expanded && (
          ...
          >
            <div className="px-4 pb-4 pt-2 bg-gray-50 border-t border-gray-200">
              ...
              {uids.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">📱 NFC UIDs</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {uids.map((uid) => (
                      <div
                        key={uid.uid}
                        className="bg-white rounded-[4px] p-3 border border-gray-200"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-mono font-medium text-purple-700">{uid.uid}</span>
              ...
              {payouts.length > 0 ? (
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">💵 Payout Transactions</h4>
                  <div className="bg-white rounded-[4px] overflow-hidden border border-gray-200">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-100">
                        <tr>
                          <th className="text-left py-3 px-4 font-medium text-gray-700">Date</th>
                          <th className="text-left py-3 px-4 font-medium text-gray-700">Order ID</th>
                          <th className="text-left py-3 px-4 font-medium text-gray-700">Source UID</th>
                          <th className="text-right py-3 px-4 font-medium text-gray-700">Total</th>
                          <th className="text-right py-3 px-4 font-medium text-gray-700">Retailer Cut</th>
                          ...
                          return (
                            <tr key={payout.id} className="hover:bg-gray-50 transition-colors">
                              ...
                              <td className="py-3 px-4">
                                {payout.source_uid ? (
                                  <span className="font-mono text-xs font-medium text-purple-700">
                                    {payout.source_uid}
                                  </span>
                                ) : (
