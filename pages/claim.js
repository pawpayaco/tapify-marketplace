    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 sm:pt-32 pb-12 sm:pb-16">
        ...
        <div className="text-center mb-12">
          <div className="inline-block p-3 bg-gradient-to-r from-[#ff7a4a] to-[#ff6fb3] rounded-[4px] mb-4">
        ...
        <div className="mb-8">
          <div className="relative">
            <input
              ...
              className="w-full px-6 py-4 pl-14 text-lg rounded-[4px] border border-gray-200 focus:border-[#ff7a4a] focus:ring-4 focus:ring-orange-100 outline-none transition-all"
            />
        ...
        {/* Error */}
        {error && (
          <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-[4px]">
        ...
        <div className="space-y-4">
          {filteredRetailers.length === 0 && !error && (
            <div className="p-12 text-center bg-white border border-gray-200 rounded-[4px]">
          ...
          {filteredRetailers.map((store) => (
            <div
              key={store.id}
              className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 bg-white border border-gray-200 rounded-[4px] hover:border-[#ff7a4a] transition-all"
            >
              <div className="flex items-start gap-4 flex-1">
                <div className="flex-shrink-0 w-12 h-12 bg-white rounded-[4px] flex items-center justify-center">
                  <svg className="w-6 h-6 text-[#ff7a4a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-xl text-gray-900 mb-1">{store.name}</h3>
                  {store.address && (
              ...
              <button
                ...
                className={`
                  w-full sm:w-auto px-8 py-3 rounded-[4px] font-medium text-white transition-all
                  ${loadingId === store.id
                    ? 'bg-gray-400 cursor-wait'
                    : 'bg-gradient-to-r from-[#ff7a4a] to-[#ff6fb3] hover:scale-105 active:scale-95'
                  }
                `}
              >
                {loadingId === store.id ? (
        ...
        {filteredRetailers.length > 0 && (
          <div className="mt-12 p-6 bg-orange-50 border border-orange-200 rounded-[4px]">
            <div className="flex items-start gap-3">
